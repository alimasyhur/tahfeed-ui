                                    ...
                                    <div class="info-items">
                                        ...
                                        <div class="info-item">
                                            ...
                                            <div>
                                                ...
                                                <div class="info-value-highlight">{{ student.total || '0' }}</div>
                                            </div>
                                        </div>
                                    </div>
                                </v-card-text>
                    ...
                    <v-card class="modern-card chart-card" elevation="6">
                        ...
                        <v-card-text class="pa-6">
                            <div class="chart-container">
                                <canvas ref="reportChart" height="300"></canvas>
                            </div>
                        </v-card-text>
                    </v-card>
                </v-col>
        ...
        <v-footer class="bg-grey-lighten-5 text-center pa-8" v-if="student && Object.keys(student).length > 0">
            <div>
                <p class="text-body-2 text-medium-emphasis mb-2">
                    ...
                    <a :href="`/org/${student.organization_domain}`" target="_blank" class="text-decoration-none">
...
export default {
    ...
    computed: {
        ...
        avgProgress() {
            if (!this.student || !this.student.total) return 0;
            // Calculate average progress based on total hafalan
            const total = parseInt(this.student.total) || 0;
            const maxExpected = 300; // Assume 300 pages as full Quran
            return Math.min(Math.round((total / maxExpected) * 100), 100);
        }
    },
    ...
    methods: {
        ...
        async loadStudentData() {
            ...
            try {
                ...
                const data = await studentStorage.showStudentByUsername(this.slug);
        ...
        async loadReports() {
            try {
                ...
                const reportsData = await reportStorage.getPublicReports(reportsParams);
                ...
                const summaryData = await reportStorage.getPublicReportSummary(summaryParams);
                ...
                console.log('Loading progress data with params:', progressParams);

                const progressData = await reportStorage.getPublicSetoranSummary(progressParams);
        ...
        createProgressChart() {
            ...
            if (!this.$refs.progressChart) {
                console.warn('progressChart ref not found');
                return;
            }
